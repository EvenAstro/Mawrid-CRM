...
const inputCls = "h-11 w-full rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[#f8faf9] px-4 text-[14px] text-slate-700 placeholder:text-slate-400 focus:border-[#1a5c4f] focus:bg-[var(--surface-raised)] focus:outline-none focus:ring-2 focus:ring-[#1a5c4f]/15 transition";
const selectCls = "h-11 w-full rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[#f8faf9] px-4 text-[14px] text-slate-700 focus:border-[#1a5c4f] focus:bg-[var(--surface-raised)] focus:outline-none focus:ring-2 focus:ring-[#1a5c4f]/15 transition appearance-none";
...
export default function UsersPage() {
  ...
  return (
    <>
      ...
      {!loading && !canEdit && (
        <div className="mb-5 rounded-[var(--radius-md)] border border-amber-200 bg-amber-50 px-4 py-3 text-[14px] font-medium text-amber-700">
      ...
      {!loading && (
        <div className="relative mb-5">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث بالاسم أو الإيميل..." className="h-11 w-full rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-raised)] pr-11 pl-4 text-[14px] text-ink-secondary e-1 placeholder:text-muted focus:border-[#1a5c4f] focus:outline-none focus:ring-2 focus:ring-[#1a5c4f]/15" />
        </div>
      )}
      ...
      <div className="overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border-subtle)] bg-[var(--surface-raised)] e-1">
        <table className="w-full border-collapse text-left">
          <thead>
            <tr className="border-b border-[var(--border-subtle)] bg-[#f8faf9] t-caption font-semibold uppercase tracking-wider text-slate-500">
                ...
                <tr key={i} className="border-b border-[var(--border-subtle)] last:border-0">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <span className="h-10 w-10 flex-none animate-pulse rounded-[var(--radius-md)] bg-[#eef4f1]" />
                  ...
                  <td className="px-6 py-4"><span className="inline-block h-8 w-32 animate-pulse rounded-[var(--radius-sm)] bg-[#eef4f1]" /></td>
                  {canEdit && <td className="px-6 py-4"><span className="inline-block h-8 w-24 animate-pulse rounded-[var(--radius-sm)] bg-[#eef4f1]" /></td>}
                  {canEdit && <td className="px-6 py-4"><span className="inline-block h-8 w-16 animate-pulse rounded-[var(--radius-sm)] bg-[#eef4f1]" /></td>}
                  ...
                  <tr key={p.id} className="border-b border-[var(--border-subtle)] last:border-0">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {p.email && <p className="truncate t-caption text-slate-400">{p.email}</p>}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <select
                        ...
                        className="h-10 rounded-[var(--radius-md)] border border-[var(--border-subtle)] bg-[#f8faf9] px-3 t-body-sm font-medium text-slate-700 focus:border-[#1a5c4f] focus:bg-[var(--surface-raised)] focus:outline-none focus:ring-2 focus:ring-[#1a5c4f]/15 transition disabled:cursor-not-allowed disabled:opacity-50"
                      >
                    ...
                    {canEdit && (
                      <td className="px-6 py-4">
                        {p.role === "admin" ? (
                          <span className="t-caption text-slate-300">وصول كامل</span>
                        ) : (
                          <button
                            onClick={() => setPermsTarget(p)}
                            className="flex items-center gap-1.5 rounded-[var(--radius-sm)] border border-slate-200 bg-[var(--surface-raised)] px-3 py-1.5 t-caption font-semibold text-slate-600 transition hover:border-[#1a5c4f]/40 hover:bg-[#f0faf8] hover:text-[#1a5c4f]"
                          >
                    ...
                    {canEdit && (
                      <td className="px-6 py-4">
                        {me?.role === "manager" && isAdminTarget ? (
                          <span className="t-caption text-slate-300">—</span>
                        ) : (
                          <button
                            onClick={() => openEdit(p)}
                            className="flex items-center gap-1.5 rounded-[var(--radius-sm)] border border-slate-200 bg-[var(--surface-raised)] px-3 py-1.5 t-caption font-semibold text-slate-600 transition hover:border-[#1a5c4f]/40 hover:bg-[#f0faf8] hover:text-[#1a5c4f]"
                      ...
                      <td className="px-6 py-4 text-right">
                        {isSelf ? null : confirmDeleteId === p.id ? (
                          <div className="flex items-center justify-end gap-2">
                            <span className="t-caption font-medium text-slate-500">متأكد؟</span>
                            <button
                              onClick={() => deleteUser(p)}
                              disabled={deletingId === p.id}
                              className="rounded-[var(--radius-sm)] bg-red-600 px-3 py-1.5 t-caption font-semibold text-white transition hover:bg-red-700 disabled:opacity-50"
                            >
                              {deletingId === p.id ? "..." : "احذف"}
                            </button>
                            <button
                              onClick={() => setConfirmDeleteId(null)}
                              className="rounded-[var(--radius-sm)] border border-slate-200 px-3 py-1.5 t-caption font-semibold text-slate-500 transition hover:bg-slate-50"
                            >
                              إلغاء
                            </button>
                          </div>
                          ...
                          <button
                            ...
                            className="rounded-[var(--radius-sm)] px-3 py-1.5 t-caption font-semibold text-red-600 transition hover:bg-red-50"
                          >
                            حذف الحساب
                          </button>
                        )}
      ...
      {addOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setAddOpen(false)}>
          ...
          <div
            ...
            className="relative w-full max-w-[480px] rounded-[var(--radius-lg)] border border-slate-200 bg-[var(--surface-raised)] p-6 shadow-2xl"
          >
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                <h3 className="text-[18px] font-bold text-slate-900">إضافة مستخدم جديد</h3>
                <p className="mt-0.5 t-body-sm text-slate-400">أنشئ حساب مباشرة بكلمة مرور وصلاحية</p>
              </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block t-body-sm font-medium text-slate-600">الاسم الأول</label>
                ...
                <div>
                  <label className="mb-1.5 block t-body-sm font-medium text-slate-600">الاسم الأخير</label>
                  <input dir="auto" value={nf.lastName} onChange={(e) => setNf({ ...nf, lastName: e.target.value })} placeholder="محمد" className={inputCls} />
                </div>
              </div>
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-slate-600">الإيميل</label>
                <input type="email" value={nf.email} onChange={(e) => setNf({ ...nf, email: e.target.value })} placeholder="name@company.com" className={inputCls} />
              </div>
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-slate-600">كلمة المرور</label>
              ...
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-slate-600">الصلاحية</label>
              ...
              {createErr && (
                <div className="rounded-[var(--radius-md)] border border-red-100 bg-red-50 px-4 py-2.5 t-body-sm font-medium text-red-700">{createErr}</div>
              )}

              <div className="flex gap-3 pt-1">
                <button onClick={() => setAddOpen(false)} className="h-11 flex-1 rounded-[var(--radius-md)] border border-slate-200 bg-[var(--surface-raised)] text-[14px] font-semibold text-slate-600 transition hover:bg-slate-50">
                  إلغاء
                </button>
                <button
                  onClick={createUser}
                  disabled={creating}
                  className="h-11 flex-1 rounded-[var(--radius-md)] bg-[#1a5c4f] text-[14px] font-bold text-white shadow-sm shadow-[#1a5c4f]/25 transition hover:bg-[#15503f] disabled:opacity-50"
                >
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-0.5 t-body-sm text-slate-400">الاسم، الإيميل، وكلمة مرور جديدة إن رغبت</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-slate-600">كلمة مرور جديدة (اختياري)</label>
              ...
              {editErr && (
                <div className="rounded-[var(--radius-md)] border border-red-100 bg-red-50 px-4 py-2.5 t-body-sm font-medium text-red-700">{editErr}</div>
              )}

              <div className="flex gap-3 pt-1">
                <button onClick={() => setEditTarget(null)} className="h-11 flex-1 rounded-[var(--radius-md)] border border-slate-200 bg-[var(--surface-raised)] text-[14px] font-semibold text-slate-600 transition hover:bg-slate-50">
                  إلغاء
                </button>
                <button
                  onClick={saveEdit}
