...
const roleMeta: Record<Role, { label: string; cls: string }> = {
  admin: { label: "أدمن", cls: "bg-[var(--status-danger-bg)] text-[var(--status-danger-fg)] ring-1 ring-[var(--status-danger-border)]" },
  manager: { label: "مدير", cls: "bg-[var(--status-info-bg)] text-[var(--status-info-fg)] ring-1 ring-[var(--status-info-border)]" },
  sales: { label: "مندوب مبيعات", cls: "bg-[var(--status-success-bg)] text-[var(--status-success-fg)] ring-1 ring-[var(--status-success-border)]" },
};
        ...
        <div className="flex flex-wrap items-center justify-between gap-5">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 flex-none items-center justify-center rounded-[var(--radius-lg)] bg-white/10">
              <svg viewBox="0 0 20 20" fill="none" stroke="var(--surface-raised)" strokeWidth={1.8} className="h-6 w-6"><circle cx="7" cy="7" r="3" /><circle cx="14" cy="9" r="2.4" /><path d="M2.5 17c.6-3 2.4-4.8 4.5-4.8s3.9 1.8 4.5 4.8M12.8 12.4c1.7.2 3 1.6 3.5 4" strokeLinecap="round" /></svg>
            </div>
            <div>
...
>>>>>>> main
      ...
      {!loading && !canEdit && (
        <div className="mb-5 rounded-[var(--radius-md)] border border-[var(--status-warning-border)] bg-[var(--status-warning-bg)] px-4 py-3 t-body-sm font-medium text-[var(--status-warning-fg)]">هذه الصفحة للعرض فقط — تحتاج صلاحية مدير أو أدمن لتعديل الأدوار.
        </div>
      )}
                ...
                <tr key={i} className="border-b border-[var(--border-subtle)] last:border-0">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <span className="h-10 w-10 flex-none animate-pulse rounded-[var(--radius-md)] bg-[var(--border-subtle)]" />
                      <div className="flex flex-col gap-1.5">
                        <span className="h-3 w-28 animate-pulse rounded bg-[var(--border-subtle)]" />
                        <span className="h-2.5 w-36 animate-pulse rounded bg-[var(--border-subtle)]" />
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4"><span className="inline-block h-5 w-16 animate-pulse rounded-full bg-[var(--border-subtle)]" /></td>
                  <td className="px-6 py-4"><span className="inline-block h-8 w-32 animate-pulse rounded-[var(--radius-sm)] bg-[var(--border-subtle)]" /></td>
                  {canEdit && <td className="px-6 py-4"><span className="inline-block h-8 w-24 animate-pulse rounded-[var(--radius-sm)] bg-[var(--border-subtle)]" /></td>}
                  {canEdit && <td className="px-6 py-4"><span className="inline-block h-8 w-16 animate-pulse rounded-[var(--radius-sm)] bg-[var(--border-subtle)]" /></td>}
...
>>>>>>> main
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex rounded-[var(--radius-sm)] px-2.5 py-1 t-caption font-semibold ${roleMeta[p.role].cls}`}>
...
>>>>>>> main
                    ...
                    {canEdit && (
                      <td className="px-6 py-4">
                        {p.role === "admin" ? (
                          <span className="t-caption text-[var(--content-disabled)]">وصول كامل</span>
                        ) : (
                          <button
                    ...
                    {canEdit && (
                      <td className="px-6 py-4">
                        {me?.role === "manager" && isAdminTarget ? (
                          <span className="t-caption text-[var(--content-disabled)]">—</span>
                        ) : (
                          <button
                            onClick={() => openEdit(p)}
<<<<<<< HEAD
                      ...
                      <td className="px-6 py-4 text-right">
                        {isSelf ? null : confirmDeleteId === p.id ? (
                          <div className="flex items-center justify-end gap-2">
                            <span className="t-caption font-medium text-[var(--content-tertiary)]">متأكد؟</span>
                            <button
                              onClick={() => deleteUser(p)}
                              disabled={deletingId === p.id}
                              className="rounded-[var(--radius-sm)] bg-[var(--status-danger-fg)] px-3 py-1.5 t-caption font-semibold text-white transition hover:bg-[var(--status-danger-fg)] disabled:opacity-50"
                            >
                              {deletingId === p.id ? "..." : "احذف"}
                            </button>
                            <button
                              onClick={() => setConfirmDeleteId(null)}
                              className="rounded-[var(--radius-sm)] border border-[var(--border-default)] px-3 py-1.5 t-caption font-semibold text-[var(--content-tertiary)] transition hover:bg-[var(--surface-sunken)]"
                            >إلغاء</button>
                          </div>
                        ) : (
                          <button
                            onClick={() => setConfirmDeleteId(p.id)}
                            className="rounded-[var(--radius-sm)] px-3 py-1.5 t-caption font-semibold text-[var(--status-danger-fg)] transition hover:bg-[var(--status-danger-bg)]"
                          >حذف الحساب</button>
                        )}
                      </td>
                    )}
      ...
      {addOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setAddOpen(false)}>
          ...
          <div
            ...
            className="relative w-full max-w-[480px] rounded-[var(--radius-lg)] border border-[var(--border-default)] bg-[var(--surface-raised)] p-[var(--space-card-pad)] shadow-2xl"
          >
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                <h3 className="t-title-3 font-bold text-[var(--content-primary)]">إضافة مستخدم جديد</h3>
                <p className="mt-0.5 t-body-sm text-[var(--content-tertiary)]">أنشئ حساب مباشرة بكلمة مرور وصلاحية</p>
              </div>
              <button onClick={() => setAddOpen(false)} className="flex-none text-[var(--content-tertiary)] transition hover:text-[var(--content-secondary)]">
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1.5 block t-body-sm font-medium text-[var(--content-secondary)]">الاسم الأول</label>
                ...
                <div>
                  <label className="mb-1.5 block t-body-sm font-medium text-[var(--content-secondary)]">الاسم الأخير</label>
                  <input dir="auto" value={nf.lastName} onChange={(e) => setNf({ ...nf, lastName: e.target.value })} placeholder="محمد" className={inputCls} />
                </div>
              </div>
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-[var(--content-secondary)]">الإيميل</label>
                <input type="email" value={nf.email} onChange={(e) => setNf({ ...nf, email: e.target.value })} placeholder="name@company.com" className={inputCls} />
              </div>
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-[var(--content-secondary)]">كلمة المرور</label>
              ...
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-[var(--content-secondary)]">الصلاحية</label>
              ...
              {createErr && (
                <div className="rounded-[var(--radius-md)] border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-4 py-2.5 t-body-sm font-medium text-[var(--status-danger-fg)]">{createErr}</div>
              )}

              <div className="flex gap-3 pt-1">
                <button onClick={() => setAddOpen(false)} className="h-11 flex-1 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-raised)] t-body-sm font-semibold text-[var(--content-secondary)] transition hover:bg-[var(--surface-sunken)]">إلغاء</button>
                <button
                  onClick={createUser}
                  disabled={creating}
<<<<<<< HEAD
      ...
      {editTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setEditTarget(null)}>
          ...
          >
            <div className="mb-5 flex items-start justify-between gap-3">
              <div>
                <h3 className="t-title-3 font-bold text-[var(--content-primary)]">تعديل بيانات {profileName(editTarget)}</h3>
                <p className="mt-0.5 t-body-sm text-[var(--content-tertiary)]">الاسم، الإيميل، وكلمة مرور جديدة إن رغبت</p>
              </div>
              <button onClick={() => setEditTarget(null)} className="flex-none text-[var(--content-tertiary)] transition hover:text-[var(--content-secondary)]">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" className="h-5 w-5"><path d="M6 6l12 12M18 6 6 18" /></svg>
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="mb-1.5 block t-body-sm font-medium text-[var(--content-secondary)]">كلمة مرور جديدة (اختياري)</label>
              ...
              {editErr && (
                <div className="rounded-[var(--radius-md)] border border-[var(--status-danger-border)] bg-[var(--status-danger-bg)] px-4 py-2.5 t-body-sm font-medium text-[var(--status-danger-fg)]">{editErr}</div>
              )}

              <div className="flex gap-3 pt-1">
                <button onClick={() => setEditTarget(null)} className="h-11 flex-1 rounded-[var(--radius-md)] border border-[var(--border-default)] bg-[var(--surface-raised)] t-body-sm font-semibold text-[var(--content-secondary)] transition hover:bg-[var(--surface-sunken)]">إلغاء</button>
                <button
                  onClick={saveEdit}
                  disabled={savingEdit}
<<<<<<< HEAD
