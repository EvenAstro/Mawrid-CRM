...
export default function TicketsPage() {
  ...
  async function createTicket() {
    ...
    if (err) {
      ...
      toast("Could not create ticket", "error");
      return;
    }
  ...
  async function updateField(id: string, field: "status" | "priority", value: string) {
    ...
    if (err) {
      ...
      toast(`${field === "status" ? "Status" : "Priority"} updated`);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="relative">
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search tickets…" className="h-11 w-full rounded-full border border-border-light bg-white pl-10 pr-4 text-[15px] text-ink-secondary placeholder:text-muted focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20" />
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-border-light bg-white shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] text-left">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="px-6 py-16 text-center text-[15px] text-muted">Loading tickets…</td></tr>
              ) : error ? (
                <tr><td colSpan={6} className="px-6 py-10"><EmptyState icon="🔌" title="Connection error" subtitle="We couldn't load tickets." action={<Button onClick={() => { setLoading(true); load(); }}>Retry</Button>} /></td></tr>
              ) : filtered.length === 0 ? (
                ...
                filtered.map((t) => (
                  <tr key={t.id} onClick={() => setSelected(t)} className="cursor-pointer border-b border-[#f1f5f9] transition-colors last:border-0 hover:bg-mint">
                    ...
                    <td className="px-6 py-3.5"><span className={`rounded-full px-2.5 py-1 text-[13px] font-medium capitalize ${statusStyle(t.status)}`}>{titleize(t.status)}</span></td>
                    <td className="px-6 py-3.5 text-[13px] text-muted">{formatDate(t.created_at)}</td>
                    <td className="px-6 py-3.5 text-right"><button onClick={(e) => { e.stopPropagation(); setSelected(t); }} className="rounded-full border border-primary px-3 py-1.5 text-[13px] font-semibold text-primary transition hover:bg-mint">View</button></td>
                  </tr>
                ))
              )}
            </tbody>
      ...
      {/* New ticket */}
      <SlideOver open={newOpen} onClose={() => setNewOpen(false)} title="New Ticket" subtitle="Report an issue or request"
        footer={<div className="flex gap-3"><Button variant="secondary" fullWidth onClick={() => setNewOpen(false)}>Cancel</Button><Button fullWidth loading={saving} onClick={createTicket}>Create Ticket</Button></div>}>
        <div className="flex flex-col gap-5">
          <Input id="nt-title" label="Title *" dir="auto" value={nt.title} onChange={(e) => setNt({ ...nt, title: e.target.value })} placeholder="Short summary" error={ntErr} autoFocus />
          <Textarea id="nt-desc" label="Description" dir="auto" value={nt.description} onChange={(e) => setNt({ ...nt, description: e.target.value })} placeholder="Details…" />
          <div className="grid grid-cols-2 gap-4">
            <Select id="nt-type" label="Type" value={nt.type} onChange={(e) => setNt({ ...nt, type: e.target.value })}>
              <option value="issue">Issue</option>
              <option value="feature_request">Feature Request</option>
              <option value="question">Question</option>
            </Select>
            <Select id="nt-priority" label="Priority" value={nt.priority} onChange={(e) => setNt({ ...nt, priority: e.target.value })}>
              {PRIORITIES.map((p) => <option key={p} value={p} className="capitalize">{titleize(p)}</option>)}
            </Select>
          </div>
        </div>
      </SlideOver>
      ...
      {/* Ticket detail */}
      <SlideOver open={!!selected} onClose={() => setSelected(null)} title={selected?.title || "Ticket"} subtitle={titleize(selected?.type ?? null)}>
        {selected && (
          <div className="flex flex-col gap-5">
            <div className="grid grid-cols-2 gap-4">
              <Select label="Status" value={(selected.status ?? "open").toLowerCase()} onChange={(e) => updateField(selected.id, "status", e.target.value)}>
                {STATUSES.map((s) => <option key={s} value={s}>{titleize(s)}</option>)}
              </Select>
              <Select label="Priority" value={(selected.priority ?? "medium").toLowerCase()} onChange={(e) => updateField(selected.id, "priority", e.target.value)}>
                {PRIORITIES.map((p) => <option key={p} value={p}>{titleize(p)}</option>)}
              </Select>
            </div>
            <div>
            ...
            <div className="rounded-xl border border-border-light bg-white p-4 text-[13px] text-muted">
              ...
              {selected.closed_at && <> · Closed {formatDateTime(selected.closed_at)}</>}
            </div>
          </div>
        )}
      </SlideOver>
